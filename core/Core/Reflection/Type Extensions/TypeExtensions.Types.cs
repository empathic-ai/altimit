...
namespace Altimit
{
    ...
    {
        public static APairedDictionary<string, Type> TypesByName { get; } = new APairedDictionary<string, Type>();
        ...
        public const string AltimitTempAssemblyName = "Altimit.Temp";
        ...
        static bool IsValidAssembly(Assembly assembly)
        {
            return assembly.GetName().Name != AltimitTempAssemblyName &&
                    (assembly.GetName().Name.StartsWith("Altimit") || assembly.GetReferencedAssemblies().SingleOrDefault(x => x.Name.ToUpper() == AltimitCoreAssemblyName.ToUpper()) != null);
        }
        ...
        public static Type GetTypeByName(string name)
        {
            ...
            {
                var tt = assembly.GetTypes().SingleOrDefault(x=> x.Name == name);
                if (tt != null)
                {
                    return tt;
                }
            ...
            throw new Exception($"Failed to find type named {name}!");
        }

        public static Type GetTypeByAName(string typeName)
        {
            try
            {
                Type type;
                if (!TypesByName.TryGetByFirst(typeName, out type))
                {
                    ...
                    if (firstBracketIndex != -1)
                    {
                        ...
                        Type[] typeArgs = innerTypeNames.Select(x => GetTypeByAName(x)).ToArray();
