    ...
    public partial class AppConnection : IAppConnection
    {
        ...
        public async Task AddOrTrackInstance(AID instanceID)
        {
            if (OS.LogReplication)
                App.Logger.Log($"Tracking instance with an ID of {instanceID}.");
            var instance = await App.InstanceDB.GetInstanceAsync(instanceID);
            Lock();
        ...
        */
        ...
        public bool TryAddAndTrackInstance(object instance)
        {
            ...
            if (!App.InstanceDB.HasInstance(instance))
                App.InstanceDB.AddInstance(instance);
        ...
        */
        ...
        public async Task EnsureTracked(object o, HashSet<object> triedObjects, CancellationToken cancellationToken)
        {
            ...
            if (TypeExtensions.IsReplicatedInstance(o))
            {
                ...
                if (!IsTrackedInstance(o))
                {
                    ...
                    {
                        instanceOnboarding.AddOrGetTaskCompletionSource(o);
                        // go through async process of making sure peer has instance
                        await PeerReplicationService.AddOrTrackInstance(o.GetInstanceID());
                    ...
                    {
                        await instanceOnboarding.AddOrGetTask(o, TimeSpan.FromSeconds(5));
                        cancellationToken.ThrowIfCancellationRequested();
                    }
                    TryTrackInstance(o);
                }
        ...
        void TrackInstance(object instance)
        {
            ...
            */
            ...
            if (OS.LogReplication)
                App.Logger.Log($"Tracking instance with an ID of { instance.GetInstanceID()}, letting peer know.");
        ...
        */
        ...
        void UntrackInstance(AID instanceID, object instance)
        {
            ...
            if (App.GetAppID(instanceID).Equals(PeerAppID))
