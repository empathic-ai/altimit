    ...
    public class P2PClientAM : AppModule<P2PClientSM>
    {
        ...
        async void OnReplicationInitialized()
        {
            try
            {
                ...
                selfSession.AddModule<ReplicationSM>();
                ...
                if (OS.LogApps)
                    Logger.Log("P2P client connecting to signaling server...");
                ...
                if (OS.LogApps)
                    Logger.Log("P2P client listening...");
                ...
                AddSession(Guid.Empty, session);

                // Add P2PClientSM and let peer know about this app's ID
                P2PClientSM = await session.Get<ReplicationSM>().Connect<P2PClientSM>();
                cancellationToken.ThrowIfCancellationRequested();
            ...
            {
                OS.Log("Peer client initialization cancelled.");
            }
        }
        ...
        {
            onSessionRemoved?.Invoke(session.Get<ReplicationSM>().PeerAppID, session);
